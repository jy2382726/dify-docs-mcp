...
- [x] **Step 1: 读取 yzmw123 原文**

```bash
cat /tmp/yzmw123-refs/usecase-node-selection.md
```

- [x] **Step 2: 读取 SKILL.md 中需要下沉的内容**
...
- [x] **Step 1: 读取当前 SKILL.md**

```bash
cat .claude/skills/dify-workflow/SKILL.md
```

- [x] **Step 2: 创建重写后的 SKILL.md**
...
- [x] **Step 4: 验证 SKILL.md 中引用的所有 reference 文件存在**
...
- [x] **Step 5: 验证 SKILL.md 包含 frontmatter 和 3 层 Reference Map**
...
- [x] **Step 6: 用 validate_dsl.py 校验所有模板和示例**
...
- [x] **Step 1: 读取当前 CLAUDE.md**

- [x] **Step 2: 更新 references/ 目录说明**
...
全部 8 个 Task 已完成。两轮评估修复（commit `bf4e2a2` 和 `0db3fae`）补充了中文化、示例一致性和字段清理。
